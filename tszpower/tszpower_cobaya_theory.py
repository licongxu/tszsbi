...
class tSZ_PS_Theory(Theory):
    ...
    def initialize(self):
        ...
        tszpower.classy_sz.set(initial_pars)
    ...
    def calculate(self, state, want_derived=True, **params_values):
        ...
        # Update the existing parameter dictionary.
        tszpower.classy_sz.pars.update(updated_pars)
        
        # Retrieve the multipole array and compute the tSZ power spectrum.
        ell = tszpower.get_ell_range()
        cl_total = tszpower.compute_integral(params_values_dict=updated_pars)
        cl_total = cl_total * ell * (ell + 1) / (2 * np.pi) * 1e12

        # For this example, assume the entire signal is from the 1-halo term.
        cl_1h = cl_total
        cl_2h = np.zeros_like(cl_total)
