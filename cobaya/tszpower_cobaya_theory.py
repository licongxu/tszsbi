...
class tSZ_PS_Theory(Theory):
    ...
    def initialize(self):
        ...
        # --- Warm-up the key JAX functions only once ---
        if not hasattr(self, '_warmup_done'):
            # Prepare a set of parameters for warmup; you can choose representative values.
            warmup_params = self.fixed_params.copy()
            ...
            # Set up tszpower with these parameters.
            tszpower.classy_sz.set(warmup_params)
            tszpower.classy_sz.compute_class_szfast()
            # Call the warmup routine to compile compute_integral and compute_trispectrum.
            # (If you don't need trispectrum, you could comment that out.)
            tszpower.warmup()
            self._warmup_done = True
            self.log.info("Warm-up call completed; JAX functions compiled.")
    ...
    def calculate(self, state, want_derived=True, **params_values):
        ...
        # 2) Set up tszpower with the current parameters.
        ...
        # Since the functions are already compiled during initialization, the overhead here is minimal.
        C_ell = tszpower.compute_integral(params_values_dict=allpars)
        # Multiply by ell(ell+1)/(2pi) and scale by 1e12.
        C_ell = C_ell * ell * (ell + 1) / (2 * jnp.pi) * 1e12
        ...
        end_time = time.time()
        self.log.info(f"Calculation completed in {end_time - start_time:.4f} seconds")
